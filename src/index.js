...
app.put('/todos/:id', checksExistsUserAccount, (request, response) => {
  ...
  const alterTodo = user.todos.find(todo => todo.id === id)
  if(!alterTodo) return response.status(404).json({ error: 'Todo not found' })
...
app.delete('/todos/:id', checksExistsUserAccount, (request, response) => {
  ...
  const excludeTodo = user.todos.find(todo => todo.id === id)
  const todoIndex = user.todos.indexOf(excludeTodo)
